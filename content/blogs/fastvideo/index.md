...
- **End users** (artists, creators, developers) want access to diverse video models with fast performance through a simple, consistent API that's easy to use and integrate with
- **Optimization Researchers** develop performance and efficiency improvements that currently must be manually integrated into each model separately, creating duplicated effort and code across the ecosystem
...
**No current framework adequately serves any of these groups.** This fragmentation forces painful tradeoffs - users sacrifice performance for model variety, optimization researchers waste effort reimplementing the same techniques across models, and model authors struggle to easily integrate performance improvements without breaking their architectures.

## FastVideo V1 directly addresses these limitations:
...
It is easy to use different attention backend configured through an environment variable. 
...
Optimization researchers face a significant challenge: implementing the same optimization technique (like a faster attention algorithm) across multiple video models requires duplicating work for each model architecture. FastVideo solves this with a pluggable attention system where:
...
class NewDiT(BaseDiT):
    ...
    def forward(self, x, attention_mask=None):
        # Attention automatically uses the configured backend
        # with the right parallelism strategy
...
This approach empowers researchers to experiment with novel attention algorithms by simply adding new backend implementations. When they create a new backend (like implementing a more efficient sparse attention pattern), they can:
...
**For End Users**: Limited to data parallelism (cannot accelerate single video generation) and requires CLI commands outside Python.

**For Optimization Researchers**: No modular architecture for optimization integration, requiring duplicated effort and complex code changes across each diffusion pipeline.
...
**For End Users**: Requires complex CLI execution with `torchrun`, manual configuration of multiple parallelism strategies, and lacks a standardized Python API. Each model requires different settings.

**For Optimization Researchers**: No universal abstraction for optimizations, which remain tightly coupled to specific model implementations.
...
Putting it all together, FastVideo's V1 API cleanly handles all the requirements.

{{< case_study >}}
Using FastVideo is dramatically simpler than other frameworks:
...
def main():
    # Can easily change which attention backends to use
    os.environ['FASTVIDEO_ATTENTION_BACKEND'] = "SAGE_ATTN"

    # Simple, unified API with built-in multi-GPU support
    model = "FastVideo/FastHunyuan-diffusers"
    generator = VideoGenerator.from_pretrained(
        model,
        num_gpus=4  # Automatic multi-GPU distribution
    )

    # params will be populated with optimal default arguments for FastHunyuan-diffusers
    param = SamplingParam.from_pretrained(model)
    param.num_inference_steps = 6

    # Generate with a single call
    video = generator.generate_video(
...
if __name__ == '__main__':
    main()
```
No need for torchrun commands:
```bash
$ python basic.py
```
{{< /case_study >}}

Next we discuss the design of FastVideo V1' architecture and how this solution is realized.

## The FastVideo V1 Architecture

{{< justify >}}
FastVideo V1 is built around a **modular component system** that separates model logic from execution strategy:

1. **Pipeline System** - Orchestrates the generation process through standardized stages
2. **Model Components and Layers** - Interchangeable and reusable neural network layers that allow transformer models, VAEs, encoders, and schedulers to benefit from all existing optimizations whether it is TP, SP, Sparse Attention kernels, and more!
3. **Optimized Attention** - Multiple backend implementations that can be selected at runtime
4. **Distributed Processing** - Tensor and sequence parallelism for efficient multi-GPU scaling

This architecture enables both simple usage for end users and extensibility for researchers:
{{< /justify >}}

### For Users: Simplicity Meets Performance

{{< justify >}}
**One Framework for All Video Models**

FastVideo V1 provides a unified interface for state-of-the-art video models. Instead of learning different APIs for each implementation, users interact with a consistent interface that handles the underlying complexity.

**State-of-the-Art Attention Backends**

At the core of FastVideo's performance improvements:

- **Sliding Tile Attention (STA)**: Accelerates video generation by up to 3× without quality compromise
- **SageAttention**: Alternative backend with different performance characteristics

**Multi-GPU Support**

FastVideo V1 scales linearly across multiple GPUs through automatic model sharding with sequence parallelism (SP) and tensor parallelism (TP). This efficiently distributes workloads and reduces per-GPU memory requirements, enabling faster generation and support for larger models.

**Python SDK and Worker Abstraction**

Our Python SDK provides a clean, intuitive interface for incorporating video generation into applications, while the worker abstraction layer handles resource management and scheduling.

Try our [Gradio Demo](https://github.com/hao-ai-lab/FastVideo/demo) to experience FastVideo without writing code.
{{< /justify >}}

### For Model Authors and Researchers: Extensibility and Performance

{{< justify >}}
**Pipeline Class for Easy Model Integration**
...
class MyCustomPipeline(ComposedPipelineBase):
    ...
    def create_pipeline_stages(self, fastvideo_args: FastVideoArgs):
        ...
        self.add_stage("prompt_encoding_stage", CLIPTextEncodingStage(
...
FastVideo's pipeline abstraction allows researchers to reuse existing diffusion code while incorporating their optimizations. The system is designed for easy integration of new features and backends without requiring complete reimplementation.
...
```python
# Tensor parallelism with optimized linear layers
