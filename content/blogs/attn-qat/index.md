...
During debugging, we found agents such as Claude surprisingly effective even for low-level PTX and CuTeDSL code. It surfaced an obscure [uninitialized register bug in FA4](https://github.com/hao-ai-lab/flash-attention-fp4/blob/c07a913f29447ff7cd4feeb95cd60704983df526/flash_attn/cute/blackwell_helpers.py#L789), when there are not enough tiles to be placed before the first mbar split. Claude cut down at least 1-2 weeks of debugging time—these tools are particularly useful for SASS inspection (e.g. CuTeDSL -> PTX → SASS mapping), instruction dependency analysis, and guided performance debugging via structured task lists in a `.md` file
