...
FastH3 Preview v1 for text-to-video-and-audio (T2VA). It distills MiniMax H3 from 
49 DiT calls to four, generates up to 768p video with
...
fast motion remains weaker than Base H3.
...
“Data-free” (also known as backward simulation in DMD2) means training uses only prompts but no target videos. The synthetic
variants use videos generated by Base H3. The dense variant provides a full-attention comparison.
We want to thank [Nuva Lab](https://nuvalab.ai/) for contributing the prompt suite and most of the
...
Currently, all four checkpoints support T2VA only. H3 uses the base transformer for both
...
We recommend trying with four NVIDIA B200 GPUs with CUDA 13. The first
