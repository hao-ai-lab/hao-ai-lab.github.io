...
| Method | Drafting Style |  Draft Cost   | Causal Path Conditioning | Tree Quality |
|:--|:--:|:--:|:--:|:--:|
| **AR baseline** |  None                            |   N/A   |   ✅   | N/A |
| **AR draft heads** |  multiple-pass sequential  	 |  💰💰💰  |   ✅   |  😃   |
| **Block-diffusion heads** | one-pass block draft   |  💰     |   ❌   |   😐  |
| **JetFlow** | one-pass causal tree draft 	         |  💰	   |   ✅   |   😃  |
...
{{< justify >}}
This leads to the central question behind JetFlow: can we draft an entire speculative tree in one parallel pass while still scoring branches according to the target model’s causal, autoregressive factorization? **JetFlow answers yes** by combining causal parallel tree drafting with one-pass verification by the frozen target model.
{{< /justify >}}

{{< justify >}}
At a high level, JetFlow targets both sides of the speculative-decoding bottleneck:
{{< /justify >}}
...
{{< justify >}}
Both JetFlow and diffusion-style draft heads can spend the same draft budget; the difference is the quality of the tree that budget buys. A block-diffusion head produces all positions from one shared hidden state with no causal mask between depths, so a depth-2 distribution may never condition on the token chosen at depth 1. When two independently likely tokens cannot follow each other, the surrogate can still promote their composition to the top of the tree. JetFlow’s causal mask anchors each position to its branch prefix, so the verifier sees branches the target is more likely to accept.
{{< /justify >}}

{{< image src="img/tree_construction_dark.gif" alt="jetflow-treewalk" width="100%" title="Figure 4: One JetFlow drafting round. The draft head keeps multiple continuations in one pass; the frozen target verifies the whole tree and accepts the longest matching prefix.">}}
...
{{< justify >}}
JetFlow is evaluated on math, coding, and open-ended chat benchmarks with Qwen3-8B and Qwen3-30B-A3B under both greedy and sampling settings. The headline pattern is budget scaling: as the draft budget grows, JetFlow keeps converting extra candidate nodes into longer accepted prefixes, while prior heads saturate earlier.
{{< /justify >}}

{{< justify >}}
At high token budget regime (>64 tokens), JetFlow leads DFlash-T (denotes to tree-based variant of DFlash, our implementation follows [DDTree](https://arxiv.org/pdf/2604.12989)) and DFlash on every benchmark. The full grid below reports end-to-end speedup (Spd) mean accepted length (τ) on Qwen3-8B over a selection of tasks.
{{< /justify >}}
...
| DFlash-T    |      64 |      6.51 (7.16) |     6.40 (6.96) |     5.08 (5.57) |     4.99 (5.49) |     5.47 (6.06) |     3.74 (4.51) |
| DFlash-T    |     128 |      8.27 (9.19) |     7.93 (8.66) |     5.93 (6.52) |     5.70 (6.28) |     6.42 (7.25) |     4.12 (5.14) |
| DFlash-T    |     256 |      8.78 (9.81) |     8.33 (9.24) |     6.31 (6.96) |     6.09 (6.70) |     6.75 (7.72) |     4.26 (5.41) |
| JetFlow     |      64 |      6.76 (7.42) |     6.47 (7.00) |     5.53 (6.06) |     5.34 (5.88) |     5.95 (6.59) |     3.97 (4.77) |
| JetFlow     |     128 |      8.93 (9.95) |     8.26 (9.10) |     6.66 (7.28) |     6.31 (6.95) |     7.29 (8.21) |     4.37 (5.52) |
| **JetFlow** | **256** | **9.64 (10.76)** | **8.78 (9.82)** | **7.12 (7.78)** | **6.73 (7.43)** | **7.67 (8.79)** | **4.58 (5.94)** |


{{< image src="" alt="" width="100%" title="Table 2: Comparison between JetFlow and other SD methods on Qwen3-8B, mean accepted length (in parentheses) and end-to-end speedup on a single H100 GPU are reported.">}}

**Takeaways:**

- **Generalizability:** largest gains appear on reasoning-heavy math and coding tasks, in consistent with our training data choice. JetFlow also generalizes with >4x speedup on open-ended conversational tasks.
...
- **Budget scaling:** larger tree budgets help JetFlow more reliably because the draft tree stays branch-conditioned.
...
{{< justify >}}
While the speedups reported are end-to-end and lossless, they depends on GPU compute, memory bandwidth, batching, and the chosen tree budget. In addition to the algorithm konbs and hardware specifications, wall-clock serving speedups run below pure algorithmic accepted-length ratios because real serving includes kernel-launch, KV cache management, scheduling and other host-side overhead. The best draft budget is a trade-off: a larger tree raises accepted length but also increases per-round verification cost, so the optimal point depends on the model and hardware.
{{< /justify >}}



##  JetFlow Serving Optimization and vLLM Integration

{{< justify >}}
To provide an inference path for real serving rather than only offline algorithmic evaluation, JetFlow includes a serving implementation based on [our vLLM fork with JetFlow support](https://github.com/Jet-Flow/vllm-jetflow) and organizes draft candidates as a speculative tree, stores parent indices along with node depths, and passes this metadata into target verification. The target then verifies all speculative nodes in one forward pass under a tree-attention mask, and the acceptance rule remains lossless by construction.
{{< /justify >}}
...
{{< image src="img/engine_demo_dark.gif" alt="jetflow_engine_demo" width="100%" title="Figure 5: JetFlow deployed on vLLM using a single B200 with Qwen3-8B, MATH-500, batch 1, budget 127, providing a throughput at around 1000 TPS on MATH-500.">}}
...
{{< justify >}}
For more details, please see the JetFlow paper and codebase:
{{< /justify >}}

- Paper: https://arxiv.org/pdf/2606.18394
- Website and Demos: https://jet-flow.github.io/
- GitHub: https://github.com/hao-ai-lab/JetFlow
- Model Zoo: https://huggingface.co/JetFlow


{{< justify >}}
Because JetFlow keeps the target model frozen and verifies under a lossless acceptance rule (speculative decoding), with a [plug-and-play causal parallel drafting head](https://huggingface.co/JetFlow), it can be attached to an existing model as a speculative decoding path while preserving the target output distribution.
{{< /justify >}}
